"""
LH Nautical - Question 7 Recommendation System Script
...
# Build mapping from order_item -> product_id and customer_id
...
# Create binary interaction matrix (Customer x Product)
...
# Calculate Cosine Similarity between Products (Columns of binary_matrix)
# Cosine similarity between columns: (X^T * X) / (norm_vector * norm_vector^T)
product_vectors = binary_matrix.values # (num_customers, num_products)
dot_product = np.dot(product_vectors.T, product_vectors) # (num_products, num_products)
norms = np.linalg.norm(product_vectors, axis=0) # (num_products,)
